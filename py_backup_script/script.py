...
s3_client = client = boto3.client('s3', aws_access_key_id=config.get("AWSAccessKey"), aws_secret_access_key=config.get("AWSSecretKey"))
...
for file in config.get("FilesToBackup"):
    ...
    try: 
        ...
        with zipfile.ZipFile(path_zip, mode='w', compression=zipfile.ZIP_DEFLATED, allowZip64=True, compresslevel=6) as zip:
            zip.write(path)
        logging.debug("---Zip Time %s seconds ---" % (time.time() - zip_time))
        logging.info("Uploading file: "+path_zip)
        try:
            upload_time = time.time()
            response = s3_client.upload_file(path_zip, config.get("BackupBucket"), filename_zip)
